...
/* Per-route spend and abuse limits (requests per minute per IP), shared by the local dev server and
   ...
   Two tiers protect the paid realtime session, because they protect different things:
     * here, `realtime/connect` = 6 REQUESTS per minute per IP. A flood guard on the HTTP route, so
       a malformed offer, an auth failure and one legitimate retry all still fit inside it.
     * in lib/voice.js, OPENAI_REALTIME_CONNECT_PER_MIN = 4 SESSIONS per minute per IP (default),
       counted only when a session is actually opened against OpenAI, plus a per-email concurrent
       call cap and a per-email daily cap. That is the control that bounds spend.
   Both are in-memory and therefore per process: on serverless they are best effort, not global.
   See the strength notes above REALTIME_CALLS in lib/voice.js. */
const RATE_PER_MIN = { turn: 40, session: 30, transcribe: 30, speak: 30, 'realtime/connect': 6, 'realtime/tool': 120, 'realtime/end': 30 };
...
async function handleVoice(sub, body, opts) {
  ...
  if (sub === 'session') {
    ...
    /* Truthful readiness: the client only attempts a live session when this server will actually
       open one. A missing key, the kill switch, or an unusable token-signing secret all report
       enabled:false with a reason, so the call quietly runs step by step instead of failing. */
    const ready = voice.realtimeReadiness(env, safeCfg);
    return json(200, {
      ok: true,
      ...
      realtime: {
        enabled: safeCfg.realtime.enabled && ready.ok,
        why: ready.ok ? safeCfg.realtime.why : ready.reason,
        config_ok: ready.ok,
        config_reason: ready.ok ? '' : ready.reason,
        ...
        max_tool_calls: safeCfg.realtime.maxToolCalls,
        idle_min: safeCfg.realtime.idleMin,
        limits: {
          connect_per_min: safeCfg.realtime.connectPerMin,
          max_concurrent: safeCfg.realtime.maxConcurrent,
          daily_max: safeCfg.realtime.dailyMax
        },
  ...
  if (sub === 'realtime/connect') {
    ...
    const ip = String((opts && opts.ip) || 'unknown');
    /* Fail closed before a single cent is spent. A forgeable session token would let anyone open a
       paid live session, so an unusable PS_TOKEN_SECRET blocks this route in production. The reason
       names the problem and never the secret value. */
    const ready = voice.realtimeReadiness(env, cfg);
    if (!ready.ok) {
      console.error('[voice] refusing to open a live session (' + ready.code + '): ' + ready.reason);
      return json(200, { ok: false, fallback: 'turns', config: ready.code, error: ready.reason });
    }
    /* Abuse controls on opening a paid session: sessions per minute per IP, concurrent live calls
       per email, sessions per email per day. Best effort per process - see lib/voice.js. Every
       refusal still returns 200 with fallback:'turns', so the call carries on step by step rather
       than ending: a limit is never the visitor's fault to fix. */
    const reg = voice.registerRealtimeCall({ email, callId, ip, cfg, env });
    if (!reg.ok) {
      console.warn('[voice] live session refused (' + reg.code + ') ip=' + ip + ' call=' + callId);
      return json(200, { ok: false, fallback: 'turns', limit: reg.code, error: reg.reason, retry_after_s: reg.retryAfterS || null });
    }
    try {
      const out = await voice.connectRealtime({
        env, fetchImpl, cfg, sdp: String(body.sdp || ''),
        // The handshake's own short timeout, not VOICE_TIMEOUT_MS: this runs inside a serverless
        // function whose platform limit (10s on Netlify's default plan) is shorter than 45s.
        timeoutMs: cfg.realtime.timeoutMs,
        ctx: {
      ...
      console.log('[voice] realtime session opened: model=' + out.model + ' voice=' + out.voice +
        ' vad=' + cfg.realtime.vad + ' call=' + callId + ' ip=' + ip +
        ' limits=' + cfg.realtime.connectPerMin + '/min,' + cfg.realtime.maxConcurrent + ' concurrent,' +
        cfg.realtime.dailyMax + '/day,' + cfg.realtime.maxSessionMin + 'min' +
        (out.attempts && out.attempts.length ? ' attempts=' + out.attempts.length : ''));
      return json(200, {
        ...
        max_tool_calls: cfg.realtime.maxToolCalls,
        idle_min: cfg.realtime.idleMin,
        call_id: callId,
        // Stamped with the authoritative start time: every later re-issue carries the same one, so
        // the session clock cannot be reset by a modified client.
        call_ticket: voice.issueRealtimeTicket(email, callId, 0, 0, reg.startedAt),
      ...
      // The session was never opened, so give the allowance back: a genuine retry must not be
      // punished for OpenAI's failure.
      voice.releaseRealtimeCall(email, callId, cfg);
  ...
  if (sub === 'realtime/end') {
    const valid = validateTurnBody(body);
    if (!valid.ok) return json(400, { ok: false, error: valid.error });
    const out = voice.endRealtimeCall({ env, body, email });
    return json(200, out);
  }
