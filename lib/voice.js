...
const INTAKE_PLAN = [
  {
    ...
    ask: 'Hi, I am Otto from PipelineSync. Let us get to know your business. What do you do, and who do you sell to?',
...
/* ------------------------------------------------------------------ */
/* Stopping: the lead's word is final                                  */
/* ------------------------------------------------------------------ */
/* The model hears the tone and reports a stop itself (stop_requested / end_call with reason
   lead_asked_to_stop); these patterns are the server's own second pair of ears, and they are
   deliberately narrow. A bare "stop" inside a sentence about the pipeline ("we stop losing
   deals") must never end a call, so only a whole intent matches: an explicit request, an
   announcement that they are leaving, or one of the fixed closing phrases. */
const STOP_END_PATTERNS = [
  /\b(?:can|could|shall) we (?:stop|end|finish|wrap(?: this)? up|leave it|call it)\b[^a-z]*(?:(?:here|now|please|thanks|thank you|sorry|for now|for today|for a (?:sec|second|moment|minute|bit))[^a-z]*)*$/i,
  /\b(?:let'?s|let us|i(?:'?d like to| want to| need to| have to| gotta| got to)|we(?:'?d like to| need to| have to| gotta| got to))\s+(?:stop|end|finish|wrap(?: this)? up|go|leave|get going)\b/i,
  /\b(?:stop|end|finish|wrap up|close)\s+(?:the|this)\s+(?:call|interview|conversation|session)\b/i,
  /\b(?:hang up|no more questions|call it a day|that(?:'?s| is) all for (?:now|today)|i(?:'?m| am) done for (?:now|today)|we(?:'?re| are) done for (?:now|today)|that(?:'?s| is) enough for (?:now|today))\b/i
];
const STOP_HOLD_PATTERNS = [
  /\b(?:hold on|hang on|give me (?:a|one) (?:moment|second|minute)|one moment|just a (?:moment|second|minute)|bear with me)\b/i,
  /\b(?:can we|let'?s|could we) (?:pause|hold|wait|break)\b|\bpause (?:for|the call|here|this)\b/i,
  /\b(?:not (?:right )?now|later today|another time|call me back|come back to this|pick this up later|reschedule)\b/i
];
/* 'end' | 'hold' | null. A long turn is an answer, not an instruction, so it is never read for one. */
function stopIntent(text) {
  const s = String(text == null ? '' : text).trim();
  if (!s || s.length > 300) return null;
  for (const re of STOP_END_PATTERNS) if (re.test(s)) return 'end';
  for (const re of STOP_HOLD_PATTERNS) if (re.test(s)) return 'hold';
  return null;
}
...
function turnSchema() {
  return {
    ...
    schema: {
      ...
      required: ['say', 'ask_question_id', 'answered_their_question', 'deferred', 'stop_requested', 'answer_quality', 'captured'],
      properties: {
        say: { type: 'string' },
...
async function openaiTurn(opts) {
  ...
  return {
    ...
    stop_requested: !!parsed.stop_requested,
...
/* ------------------------------------------------------------------ */
/* What the AI says when the lead stops the call and the model did not hand us a closing line of its
   own (the server's own stop detector fired). It closes warmly and asks nothing. */
function stopLine(ctx) {
  const first = String((ctx && ctx.clientName) || '').trim().split(/\s+/)[0] || '';
  return 'Of course, we will stop here' + (first ? ', ' + first : '') +
    '. Everything you have told me is saved, and you can pick this up whenever you are ready. Thank you for your time.';
}

/* The built-in interviewer (no OPENAI_API_KEY) has no model to notice that the lead just asked
   something, so it matches their words against the same scripted FAQ the live session carries:
   answer first (KB-CALL-01), and leave the assigned question for the next turn. Matching is on
   the lead's own words and only picks a scripted answer, so nothing is invented here either. */
const SIM_FAQ = [
  { match: /\bquestions? (are )?left\b|\bhow many (more )?questions\b|\bhow many more\b/i, q: 'How many questions are left?' },
  { match: /\bhow long\b|\bhow much time\b|\bhow long will\b/i, q: 'How long is this?' },
  { match: /\b(what|who) (is|are) (this|pipelinesync|you)\b|\bwho are you\b|\bwhat do you (guys|all) do\b/i, q: 'Who or what is PipelineSync?' },
  { match: /\breal (person|human)\b|\bare you (a |an )?(ai|robot|bot|human|machine|person)\b|\b(is this|am i talking to|are you) a (robot|bot|machine)\b|\brobot\b/i, q: 'Are you a real person?' },
  { match: /\b(cost|price|charge|pricing|free|how much (do|does|is|would|will))\b/i, q: 'What does it cost, what do you charge?' },
  { match: /\bwhat happens (after|next|now)\b|\bafter (the|this) call\b|\bwhat do i get\b/i, q: 'What happens after the call?' },
  { match: /\bdo i need hubspot\b|\balready (on|use|using) hubspot\b/i, q: 'Do I need HubSpot already?' },
  { match: /\b(data|privacy|private|safe|secure|confidential|store|keep (it|my))\b/i, q: 'Is my data safe, who sees it?' },
  { match: /\b(speak|talk) to (a )?(human|person|someone|team)\b|\bhuman (review|call|follow)/i, q: 'Can I speak to a human?' },
  { match: /\b(change|edit|correct|fix) my answers\b|\bmy answers later\b/i, q: 'Can I change my answers?' },
  { match: /\bcan i (ask|ask you|we ask)\b|\bcan i ask (you )?(questions|things|stuff)\b/i, q: 'Can I ask you things as we go?' },
  { match: /\b(stop|finish|pause|resume|carry on) (this|later|here)\b|\bcan we (stop|finish)\b|\bfinish (this )?later\b|\bpick (this )?up later\b/i, q: 'Can we stop, or finish this later?' }
];
function faqAnswerFor(text) {
  const s = String(text == null ? '' : text).trim();
  if (!s || s.length > 300) return null;
  for (const m of SIM_FAQ) {
    if (!m.match.test(s)) continue;
    const hit = REALTIME_FAQ.find(f => f[0] === m.q);
    if (hit) return { question: hit[0], say: hit[1] };
  }
  return null;
}
...
async function runTurn(opts) {
  ...
  if (!wording) {
    const faq = faqAnswerFor(lastAnswer);
    /* Their question first: the built-in engine answers from the same scripted FAQ, and the
       assigned question waits for the next turn (a deferred turn, exactly like the model's). */
    wording = faq
      ? { say: faq.say, ask_question_id: null, deferred: true, captured: [] }
      : { say: simulatedSay(ctx), ask_question_id: step.question ? step.question.id : null, captured: [] };
  }
  ...
  /* The lead's word is final. The model reports a stop it heard; the server also reads the last
     thing the lead actually said, and either one is enough to end the call - the intake, the
     missing figures and the counts do not get a vote. */
  const heardStop = stopIntent(lastAnswer) === 'end';
  const stopping = wording.stop_requested === true || heardStop;
  if (stopping && step.question) {
    warnings.push('The lead asked to stop; the call closed with ' + (capture.missingRequired.length ? capture.missingRequired.length + ' required figure(s) still missing' : 'the intake set complete') + '.');
  }
  /* A turn that is all answer: the lead asked something and never got to the assigned question.
     The question stays pending, so the client does not record it as asked and it comes back next
     turn. That is the whole point of an interactive call: their question first, the intake after. */
  const deferred = !stopping && wording.deferred === true && !!step.question;

  const say = stopping
    ? (spokenStyle(wording.stop_requested === true ? wording.say : '', cfg.maxSayChars) || stopLine(ctx))
    : (spokenStyle(wording.say, cfg.maxSayChars) || (step.question ? step.question.ask : 'Thank you, that is everything I need.'));
  ...
  return {
    ...
    plan_version: PLAN_VERSION,
    say,
    ask: (step.question && !stopping)
      ? { id: step.question.id, label: step.question.label, kind: deferred ? 'deferred' : step.kind, field: step.field || null, fields: step.question.fields, hint: step.question.hint, intent: step.question.intent, deferred: deferred }
      : { id: null, label: stopping ? 'Stopped at the lead\'s request' : 'Call complete', kind: 'done', field: null, fields: [], hint: '', intent: '' },
    done: !step.question || stopping,
    stop_requested: stopping || undefined,
...
const REALTIME_FAQ = [
  ...
  ['Can I ask you things as we go?', 'Of course, ask me anything. I will answer it first, then we pick up where we were, and you can keep going as long as you like.'],
  ['Can we stop, or finish this later?', 'Yes. Say the word and we stop right there. Everything you have given me is saved, so you can pick it up whenever you are ready.'],
  ['How many questions are left?', 'Twelve in all, and there is no penalty for skipping any of them. Tell me to move on and I will.']
];
...
function realtimeInstructions(ctx, cfg) {
  ...
  return [
    'You are Otto, the PipelineSync AI discovery interviewer, on a live continuous voice call with ' + (name ? name + ', a business owner' : 'a business owner') + ' in the Philippines. You are an AI, and you say so once, in your opening line.',
    ...
    '',
    'THEIR QUESTION COMES FIRST. This is a conversation, so a question from the lead owns the turn:',
    ...
    '',
    'WHEN THEY SAY STOP, YOU STOP. Their word is final and needs no permission:',
    ...
    'Open the call now: greet them' + (name ? ', use their first name' : '') + ', say you are Otto, an AI interviewer from PipelineSync, say the call takes a few minutes and that they can stop any time, then ask question 1.'
  ].join('\n');
}
...
function realtimeTools() {
  return [
    ...
    },
    {
      type: 'function',
      name: 'end_call',
      description: 'End the call. Call it immediately when the lead asks to stop or says they have to go, when the intake set is complete, or when everything still outstanding has been declined. A stop is never questioned and never delayed.',
      parameters: {
        type: 'object', additionalProperties: false, required: ['reason'],
        properties: {
          reason: {
            type: 'string',
            enum: ['complete', 'lead_asked_to_stop', 'lead_declined', 'time_limit', 'other'],
            description: 'Why the call is ending. lead_asked_to_stop: the lead asked to stop, end it or leave. complete: the intake set is finished. lead_declined: the outstanding answers were declined.'
          }
        }
      }
    }
...
function runRealtimeTool(opts) {
  ...
  if (name === 'end_call') {
    ...
    /* The lead asked to stop: the model heard it and said so, or their own last words read as a
       stop. Either way the call closes here and now. Missing figures, the intake set and the call
       clock do not get a vote, and nothing asks them for one more. */
    const leadAskedToStop = reason === 'lead_asked_to_stop' || stopIntent(body.user_turn || body.last_answer) === 'end';
    if (leadAskedToStop) {
      warnings.push('The lead asked to stop: the call closed at their request' + (missing.length ? ' with ' + missing.length + ' required figure(s) still open' : '') + '.');
      return respond({
        close: true, stop_requested: true, reason: reason || 'lead_asked_to_stop',
        missing_required: missing.map(k => FIELD_LABELS[k]),
        instruction: 'The lead asked to stop, so the call is closing now. Thank them warmly in one short sentence' + (body.client_name ? ' by name' : '') + ', tell them everything they gave is saved and they can pick this up whenever they are ready, and ask nothing at all. No more questions, no asking for one more figure, and no disappointment in your voice.'
      }, { close: true, capture });
    }
    ...
    return respond({
      close: true, missing_required: missing.map(k => FIELD_LABELS[k]), reason: reason || 'complete',
...
module.exports = {
  ...
  mode, nextStep, stopIntent, faqAnswerFor, captureState, buildMessages, turnSchema, spokenStyle,
