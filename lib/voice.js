...
/* The continuous call has its own ticket: it counts tool calls (one per answer) and the seconds the
   session has been open, so a client that never hangs up is stopped by the server, not by the bill. */
function issueRealtimeTicket(email, callId, calls, seconds) {
  return core.signToken({ email, call_id: callId, turns: calls, seconds: seconds || 0, kind: 'voice-realtime', exp: Date.now() + 3 * 60 * 60 * 1000 });
}
...
function runRealtimeTool(opts) {
  ...
  const toolCalls = ticket && ticket.call_id === callId ? (ticket.turns || 0) + 1 : 1;
  const seconds = ticket && ticket.call_id === callId ? Math.max(0, Math.round((Date.now() - (ticket.iat || ticket.exp - 3 * 60 * 60 * 1000)) / 1000)) : 0;
  ...
  const respond = (output, extra) => Object.assign({
    ...
    call_ticket: issueRealtimeTicket(email, callId, toolCalls, seconds),
    state: { answers, asked, probes, skipped, voice_captures: captures.slice(-60), capture: captureNow() },
    tool_calls: toolCalls, warnings
  }, extra || {});
...
/* The call is over: recompute the contract state from everything that was captured, so the review
   screen and Function A see exactly what the live call produced. */
function endRealtimeCall(opts) {
  ...
  console.log('[voice] realtime call ended: turns=' + state.asked.length + ' captured=' + capture.filledCount + '/' + capture.totalCount +
    ' skipped=' + state.skipped.length + ' missing_required=' + JSON.stringify(capture.missingRequired) + ' audio_retained=false');
  return {
