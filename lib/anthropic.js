...
// The pinned default model. This is the ONE place a model id is hardcoded:
// everything else reads it through resolveModel(env) / ANTHROPIC_MODEL.
const DEFAULT_MODEL = 'claude-sonnet-4-5-20250929';
const DEFAULT_MAX_TOKENS = 4096;

/**
 * Resolve the model to use: ANTHROPIC_MODEL from the environment, else the
 * pinned default. Never hardcode a model id anywhere else.
 * @param {Object} [env]
 * @returns {string}
 */
function resolveModel(env) {
  const m = ((env || process.env).ANTHROPIC_MODEL || '').trim();
...
async function callClaude({ systemPrompt, userMessage, model, maxTokens, env, fetchImpl }) {
  ...
  try {
    ...
    return { ok: false, error: 'Could not reach the AI service: ' + e.message, retryable: true };
  }
  ...
  if (!res.ok) {
    ...
    return { ok: false, error: errMsg, status: res.status, retryable: res.status === 429 || res.status >= 500 };
  }
...
module.exports = { isEnabled, resolveModel, callClaude, callClaudeJSON, DEFAULT_MODEL, ANTHROPIC_URL, ANTHROPIC_VERSION };
