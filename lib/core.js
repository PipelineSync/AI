...
function moneyMatches(text) {
  ...
  while ((m = re3.exec(text))) {
    ...
    if (!out.some(o => o.raw === m[1])) out.push({ value: v, raw: m[0] });
  }
  return out;
}
...
function tokenSecret() {
  return process.env.PS_TOKEN_SECRET || 'pipelinesync-prototype-dev-secret';
}
...
/* ------------------------------------------------------------------ */
function makeLeadPayload(email, name, fields, bp) {
  return {
...
module.exports = {
  KB, num, extract, generate, buildPdf, signToken, verifyToken, loginNameFor,
