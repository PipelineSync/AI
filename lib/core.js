...
/* Spoken numbers                                                      */
/* ------------------------------------------------------------------ */
/* The discovery call is voice, so answers arrive as words: "about fifty five
   leads a month", "a typical deal is one and a half million", "we close around
   thirty percent". The digit-only regexes returned null for all of those, which
   the review screen then reported as "Not stated" - a figure the client did say
   was indistinguishable from one they never gave, and it silently dropped the
   Marketing Hub rule that keys off spend. These helpers read both forms.
   Values are never invented: if no number phrase is present, null still. */
const MULT = { hundred: 100, thousand: 1000, k: 1000, million: 1e6, billion: 1e9 };
const NUM_FILLERS = ['and', 'about', 'around', 'roughly', 'approximately', 'nearly', 'just', 'under', 'over', 'say', 'maybe', 'something', 'like'];
const UNIT_FILLERS = ['is', 'are', 'was', 'were', 'be', 'about', 'around', 'roughly', 'approximately', 'nearly', 'typically', 'usually', 'most', 'each', 'per', 'at', 'of', 'the', 'a', 'an', 'on', 'spend', 'spending', 'budget'];
/* Count nouns that follow a number but are not money: "two thousand installs". */
const NOT_MONEY = ['install', 'installs', 'job', 'jobs', 'lead', 'leads', 'enquiry', 'enquiries', 'inquiry', 'inquiries', 'query', 'queries', 'contact', 'contacts', 'order', 'orders', 'unit', 'units', 'panel', 'panels', 'patient', 'patients', 'customer', 'customers', 'client', 'clients', 'deal', 'deals', 'sale', 'sales', 'booking', 'bookings', 'visit', 'visits', 'call', 'calls', 'person', 'people', 'staff', 'rep', 'reps', 'seat', 'seats', 'percent', 'day', 'days', 'week', 'weeks', 'month', 'months', 'year', 'years', 'time', 'times'];

/* Both budgets out of one spoken or typed answer. Each figure is assigned to the
   keyword it sits nearest, so in "fifty thousand a month on marketing and five
   thousand on software" the second figure cannot be read as the marketing spend
   just because it happens to be closer to the word "marketing". */
function spendFigures(text) {
  const lower = String(text == null ? '' : text).toLowerCase();
  const MARKETING = /\b(?:ads|advertising|marketing|google|facebook|meta|social)\b/g;
  const SOFTWARE = /\b(?:software|tools|subscriptions|licen[cs]es?)\b/g;
  const out = { marketing: null, software: null };
  const cands = [];
  const tk = tokenize(lower);
  let cursor = 0;
  for (let i = 0; i < tk.length; i++) {
    const p = parseNumberPhrase(tk, i);
    if (!p || p.next <= i) continue;
    const raw = tk.slice(i, p.next).join(' ');
    const at = lower.indexOf(raw, cursor);
    if (at !== -1 && NOT_MONEY.indexOf(tk[p.next] || '') === -1) {
      cursor = at + raw.length;
      cands.push({ value: p.value, centre: at + raw.length / 2 });
    }
    i = p.next - 1;
  }
  const keys = [];
  let m;
  while ((m = MARKETING.exec(lower))) keys.push({ set: 'marketing', centre: m.index + m[0].length / 2 });
  while ((m = SOFTWARE.exec(lower))) keys.push({ set: 'software', centre: m.index + m[0].length / 2 });
  for (const c of cands) {
    let bestSet = null, bestDist = Infinity;
    for (const k of keys) {
      const d = Math.abs(c.centre - k.centre);
      if (d < bestDist) { bestDist = d; bestSet = k.set; }
    }
    if (bestSet && out[bestSet] == null) out[bestSet] = c.value;
  }
  return out;
}

function tokenize(text) {
  return String(text == null ? '' : text).toLowerCase().match(/\d[\d,]*(?:\.\d+)?|[a-z]+/g) || [];
}

/* Parse a number phrase starting at token i: "fifty five" -> 55, "two hundred"
   -> 200, "one and a half million" -> 1500000, "1,500,000" -> 1500000.
   Returns { value, next } or null when there is no number at that position. */
function parseNumberPhrase(tk, i) {
  let total = 0, current = 0, seen = false, scaled = false, j = i;
  while (j < tk.length) {
    const t = tk[j];
    if (/^\d/.test(t)) {
      if (seen) break;
      current += parseFloat(t.replace(/,/g, '')); seen = true; j++; continue;
    }
    if (WORDS[t] != null) { current += WORDS[t]; seen = true; j++; continue; }
    if (t === 'half') { current += 0.5; seen = true; j++; continue; }
    if (t === 'a' || t === 'an') {
      const nxt = tk[j + 1];
      if (current === 0 && nxt && MULT[nxt] != null) { current = 1; seen = true; j++; continue; }
      if (nxt === 'half') { j++; continue; }   // "one and a half million": the 1 is already in
      break;
    }
    /* "one and a half million" bridges on "and", but "one and a half million
       and three reps" must stop there: past a scaled unit the next "and" starts
       a new clause, and swallowing it both corrupts the value and drags the
       following count noun into the money match. */
    if (t === 'and') { if (scaled) break; j++; continue; }
    if (MULT[t] != null) {
      const m = MULT[t];
      if (current === 0) current = 1;
      if (m === 100) current *= 100; else { total += current * m; current = 0; scaled = true; }
      seen = true; j++; continue;
    }
    break;
  }
  return seen ? { value: total + current, next: j } : null;
}

/* The first number phrase in the text that is followed (within a few fillers)
   by one of the unit words: "about fifty five new leads" -> 55. */
function numberByUnit(text, units, maxGap) {
  const tk = tokenize(text);
  const gap = maxGap == null ? 2 : maxGap;
  for (let i = 0; i < tk.length; i++) {
    const p = parseNumberPhrase(tk, i);
    if (!p || p.next <= i) continue;
    let j = p.next, steps = 0;
    while (j < tk.length && steps <= gap) {
      if (units.indexOf(tk[j]) !== -1) return p.value;
      if (NUM_FILLERS.indexOf(tk[j]) === -1) break;
      j++; steps++;
    }
  }
  return null;
...
/* The first number phrase that follows one of the unit words:
   "most jobs are around eighty thousand each" -> 80000. */
function numberAfterUnit(text, units, maxGap) {
  const tk = tokenize(text);
  const gap = maxGap == null ? 4 : maxGap;
  for (let i = 0; i < tk.length; i++) {
    if (units.indexOf(tk[i]) === -1) continue;
    let j = i + 1, steps = 0;
    while (j < tk.length && steps < gap) {
      const p = parseNumberPhrase(tk, j);
      if (p && p.next > j) return p.value;
      if (UNIT_FILLERS.indexOf(tk[j]) === -1) break;
      j++; steps++;
    }
  }
  return null;
...
function moneyMatches(text) {
  ...
  function addMatch(value, raw, index, length) {
    ...
    out.push({ value, raw, index, end: index + length });
  }
  let m;
  ...
  while ((m = re2.exec(text))) {
    ...
    /* Spoken money: "fifty thousand", "one and a half million", "about eighty k".
       An explicit multiplier is required, and a following count noun vetoes the
       match, so "two hundred leads a month" is never read as money. */
    const lower = String(text == null ? '' : text).toLowerCase();
    const tk = tokenize(lower);
    let cursor = 0;
    for (let i = 0; i < tk.length; i++) {
      const p = parseNumberPhrase(tk, i);
      if (!p || p.next <= i) continue;
      const phrase = tk.slice(i, p.next);
      const hasMult = phrase.some(t => MULT[t] != null);
      const vetoed = NOT_MONEY.indexOf(tk[p.next] || '') !== -1;
      if (hasMult && !vetoed) {
        const raw = phrase.join(' ');
        const at = lower.indexOf(raw, cursor);
        if (at !== -1) { cursor = at + raw.length; addMatch(p.value, raw, at, raw.length); }
      }
      i = p.next - 1;
    }
    return out;
...
function extract(answersArr) {
  ...
  if (a.volumes) {
    ...
    if (fields.monthly_lead_volume == null) {
      /* spoken form: "about fifty five leads a month" */
      const spokenLv = numberByUnit(t, ['leads', 'enquiries', 'inquiries', 'contacts', 'queries']);
      if (spokenLv != null) fields.monthly_lead_volume = spokenLv;
    }
    ...
    if (fields.close_rate == null) {
      /* spoken form: "we close around thirty percent" */
      const spokenRate = numberByUnit(t, ['percent', 'cent']);
      if (spokenRate != null && spokenRate > 0 && spokenRate <= 100) fields.close_rate = spokenRate;
    }
  ...
  if (a.spend) {
    ...
    /* Word order reversed, or the figure spoken rather than typed. */
    if (fields.monthly_marketing_spend == null || fields.monthly_software_budget == null) {
      const figs = spendFigures(a.spend);
      if (fields.monthly_marketing_spend == null) fields.monthly_marketing_spend = figs.marketing;
      if (fields.monthly_software_budget == null) fields.monthly_software_budget = figs.software;
    }
